...
WaffleBox::WaffleBox(const std::string &nome,const std::string &id, u_int c, u_int w, double p, u_int d, u_int s) : Name(nome), ID(id), capacity(c), weight(w), price(p), discount(d), stockavailability(s) {}
WaffleBox::WaffleBox(const WaffleBox &w) : Name(w.getName()),ID(w.getID()), capacity(w.getCapacity()), weight(w.getWeight()), price(w.getPrice()), discount(w.getDiscount()), stockavailability(w.getStockAvailability()) {}
