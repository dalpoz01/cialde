...
//Costruttore
CircleBox::CircleBox(const std::string &nome, const std::string &id, const std::string &foto, u_int c, u_int w, double p, u_int dis, u_int s, u_int d): WaffleBox(nome, id, foto, c, w, p, dis, s), radius(d) {}

//Costruttore di copia
CircleBox::CircleBox(const CircleBox& c): WaffleBox(c.getName(),c.getID(), c.getPhoto(), c.getCapacity(), c.getWeight(), c.getPrice(), c.getDiscount(), c.getStockAvailability()), radius(c.getRadius()){}
