...
Covered::Covered(const std::string &nome,const std::string &id, u_int c, u_int w, double p, u_int dis, u_int s, u_int h, u_int d, const std::string& tst) : ConeBox(nome, id, c, w, p, dis, s, h, d), taste(tst) {}
Covered::Covered(const Covered& c): ConeBox(c.getName(), c.getID(), c.getCapacity(), c.getWeight(), c.getPrice(), c.getDiscount(), c.getStockAvailability(), c.getHeight(), c.getExtDiameter()), taste(c.getTaste()){}
