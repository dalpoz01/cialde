...
VentaglioBox::VentaglioBox(const std::string& nome,const std::string &id, u_int c, u_int w, double p, u_int d, u_int s, u_int h, u_int wd) : HeightProviderBox(nome, id, c, w, p, d, s, h), width(wd) {}
VentaglioBox::VentaglioBox(const VentaglioBox &v) : HeightProviderBox(v.getName(), v.getID(), v.getCapacity(), v.getWeight(), v.getPrice(), v.getDiscount(), v.getStockAvailability(), v.getHeight()), width(v.getWidth()) {}
